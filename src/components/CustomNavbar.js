...
const CustomNavbar = () => {
  ...
  return (
    <AppBar position="fixed" sx= {{ backgroundColor: customTheme.palette.background.default }}>
      <Toolbar>
        <Box sx={{ display: 'flex', justifyContent: 'flex-start', flexGrow: 1, backgroundColor: customTheme.palette.background.default }}>
        <img
        ...
        {isMobile ? (
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', backgroundColor: customTheme.palette.background.default }}>
            {navLinks.map((link) => (
              <Button
                key={link.name}
                color="inherit"
